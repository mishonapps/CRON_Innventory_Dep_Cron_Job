    ...
    prdct_template <- tryCatch(
      {
        prdct_template <- dbGetQuery(rds_odoodb,"select template.default_code as SKU, supplierinfo.min_qty, supplierinfo.sequence, 
                            supplierinfo.delay
                             ...
                             template.active = TRUE and template.product_status='active' and template.available_in_new_dundee = TRUE and 
...
packaging.sequence, packaging.height, packaging.width, packaging.length, packaging.is_individual, 
...
FROM dailysales WHERE GMV > 0 GROUP BY SKU"))
       ...
       !is.null(stock_Move) & !is.null(forecastData))
    {
      ...
      stock_Move2 <- stock_Move %>%group_by(sku,date)%>%summarise(order_qty = sum(product_qty),booking_reference=booking_reference[1])%>%slice(which.min(as.Date(date)))
      ...
      join1 <- left_join(prdct_template1%>%select(sku,min_qty,delivery_lead_time=delay),summ_SalesDF,by=c("sku"="SKU"))
      ...
      for(s in 1:dim(req_product)[1])
      {
        ...
        {
          # 
          # seasonalityinfo = sec_seasonality[sec_seasonality$Date == dateFormatted[s2],]
          ...
          if(round(prdctInfo$Total_on_hand-forecastInfo)  ==0)
          {
            ...
            if(remaining_Days>0)
            {
              seasonality_sum2 = sum(sec_seasonality_2$sIndex[sec_seasonality_2$id>s2 & sec_seasonality_2$id <= remaining_Days +30])
      ...
      oosDates3$Days_Untill_OOS <- as.numeric(as.Date(oosDates3$DATE) - Sys.Date())
      
      join4$Days_Untill_OOS[join4$sku %in% oosDates3$refSKUs] <-  oosDates3$Days_Untill_OOS
      ...
      fill_qty_req <- join4[join4$Product_Status=="Out of stock" & join4$booking_reference!="Not on order" & join4$Forecasted_Annual_Qty>0,]
      #
      rds_mishondb_con <- tryCatch(
        {
          rds_mishondb_con <- DBI::dbConnect(
      ...
      prdct_Packaging2 = prdct_Packaging%>%
        filter(is_individual)%>%
        group_by(default_code)%>%
        slice(which.min(sequence))%>%select(sku=default_code,height,width,length)
      
      prdct_Packaging3 = prdct_Packaging%>%
        filter(!is_individual)%>%
        group_by(default_code)%>%
        slice(which.min(sequence))%>%select(sku=default_code,Master_Carton_Qty=qty)
      
      prd_packDF <- left_join(prdct_Packaging2,prdct_Packaging3,by="sku")
      
      join4_0_1 <- left_join(join4,prd_packDF,by="sku")
      
      join4_0_1$height <- join4_0_1$height * 0.08333
      join4_0_1$width <- join4_0_1$width * 0.08333
      join4_0_1$length <- join4_0_1$length * 0.08333
      
      join4_0_2 <- left_join(join4_0_1,stock_Move%>%select(sku,forecasted_delivery_date),by="sku")
      ...
      # stock_Move$delivery_Date = ifelse(is.na(stock_Move$conf_del_date),as.character(stock_Move$org_del_date),as.character(stock_Move$conf_del_date))
      # 
      # 
      # stock_Move123 = stock_Move%>%select(sku,destination,delivery_Date)%>%spread(destination,delivery_Date,-sku)
      # 
      # 
      # 
      # join5 <- left_join(join4,stock_Move%>%select(sku,delivery_Date),by="sku")
      ...
      {
        DBI::dbGetQuery(rds_mishondb_con, "TRUNCATE TABLE oos_inventory_report")
        DBI::dbWriteTable(rds_mishondb_con,"oos_inventory_report",join4_0_2,row.names=FALSE,append=TRUE)
